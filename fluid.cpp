...
inline Vector2 operator*(float s, const Vector2 &vec) { return Vector2(vec.x*s, vec.y*s); }
inline Vector2 operator*(const Vector2 &vec, float s) { return Vector2(vec.x*s, vec.y*s); }
inline Vector2 operator/(float s, const Vector2 &vec) { return Vector2(vec.x/s, vec.y/s); }
inline Vector2 operator/(const Vector2 &vec, float s) { return Vector2(vec.x/s, vec.y/s); }
...
private:
	...
	void jacobi(T *x, T *b, float alpha, float beta, uint iters=20) {
		...
		for (uint t = 0;t < iters;++t) {
			...
			for (uint j = 1;j < H-1;++j) {
				result[i*H+j] = (x[(i-1)*H+j]+x[(i+1)*H+j]+x[i*H+j-1]+x[i*H+j+1] + alpha*b[i*H+j])/beta;
			}
	...
	void advect(Vector2 *result, float dt=1) {
		...
		for (uint y = 0;y < H;++y) {
			Vector2 vel = v[x*H+y];
			result[x*H+y] = lerp_index(Vector2(x-vel.x,y-vel.y))*dt;
		}
	}
	...
	void divergence(float *result) {
		for (uint x = 1;x < W-1;++x)
		for (uint y = 1;y < H-1;++y) {
			result[x*H+y] = (v[(x+1)*H+y].x - v[(x-1)*H+y].x + v[x*H+y+1].y - v[x*H+y-1].y) / 2;
		}
	}
...
int main() {
	...
	while (running) {
		...
		for (int j = 0; j < pb.height();++j)
			buffer[i + j*pb.height()] += buffer[i+1+(j+1)*pb.height()];
